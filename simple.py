...
class State:
    ...
    current_poly_verts: Optional[jnp.ndarray] = None
    current_actual_num_verts: Optional[jnp.ndarray] = None
    current_r: Optional[jnp.ndarray] = None # 覆盖半径
...
class SimpleMPE(MultiAgentEnv):
    def __init__(
        ...
        **kwargs,
        ...
        if action_spaces is None:
            ...
            elif action_type == FORCE2D_ACT:  # 新增分支
                self.action_spaces = {i: Box(-1.0, 1.0, (2,), dtype=jnp.float32) for i in self.agents}
        else:
            assert (
        ...
        elif action_type == FORCE2D_ACT:
            self.action_decoder = self._decode_force2d
        else:
    ...
    @partial(jax.vmap, in_axes=[None, 0, 0])
    def _decode_force2d(self, a_idx: int, action: chex.Array) -> Tuple[chex.Array, chex.Array]:
        """
        恒等解码：直接把 (fx, fy) 当作物理力。
        仍乘以 accel[i] 与 moveable[i]，与旧解码器保持一致。
        """
